...
All sub-skills are loaded via `load_skill <name>` (defined above), which cat's `$SKILL_BASE/<name>/GUIDE.md` and warns instead of failing if a sub-skill was not uploaded.
...
```bash
cat "$SKILL_BASE/ui-ux-pro-max/GUIDE.md"
```
...
```bash
cat "$SKILL_BASE/design-audit/GUIDE.md"
```
